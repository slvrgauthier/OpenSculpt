...
MCone::MCone() : m_radius1(5.0), m_radius2(0.1), m_height(5.0)
{
  ...
  for(int k =0;k<8;k++){
      float resultCos1= m_radius1*cosd(angle*k);//cos BAS
      float resultSin1= m_radius1*sind(angle*k);//sin BAS
      float resultCos2= m_radius2*cosd(angle*k);//cos HAUT
      float resultSin2= m_radius2*sind(angle*k);//sin HAUT
      m_vertices.push_back(new Vertex); //BAS
      m_vertices.last()->coords =  QVector3D(resultCos1, -m_height/2., resultSin1);
      m_vertices.last()->index = k*2+2;

      m_vertices.push_back(new Vertex); //HAUT
      m_vertices.last()->coords =  QVector3D(resultCos2, m_height/2., resultSin2);
...
void MCone::setRadiusDown(float radius){

}

void MCone::setRadiusUP(float radius){

}

void MCone::setHeight(float height){
